...
class NoteList extends Component {
    ...
    updateNavigationButtons = () => {
      this.props.navigation.setOptions({
        headerRight: (state) => (
          ...
          <TouchableOpacity style={styles.rigthNavigationButton} onPress={() => this.deleteNote()} testID="btnRemoveSelectedNotes">
            <Image
    ...
    updateNote = (updatedNote) => {
      ...
      this.setState({ notes: notes }, () => this.noteDao.saveNotesCollection(this.state.notes));
    }
    ...
    deleteNote = () => {
      ...
      this.setState({ notes: newNotesList }, () => this.noteDao.saveNotesCollection(this.state.notes));
      this.clearSelection();
    }
    ...
    goToNoteCreation = () => {
      this.clearSelection();
      this.props.navigation.navigate('NoteEdition', {note: this.generateNewEmptyNote()});
    }
    ...
    isSelectModeEnabled = () => (this.state.notes.filter(note => note.isSelected).length > 0)
    ...
    render() {
        return (
          <>
            ...
            renderItem={({item}) => <NoteView isSelected={item.isSelected} note={item} onLongSelect={(note) => this.selectNote(note)} onSelect={this.goToNoteEdition}/> }
