...
class NoteDao {
  constructor() {
    this.getLastIdGenerated(lastIdGenerated => {
      this.lastIdGenerated = parseInt(lastIdGenerated, 10);
    });
  }
  ...
  getNotesFromDatabase = async callback => {
    try {
      ...
      if (rowNotesData !== null) {
        const parsedNotesData = JSON.parse(rowNotesData);
        parsedNotesData.forEach(note =>
          notes.push(new Note(note.id, note.title, note.description))
        );
      }
      callback(notes);
    } catch (e) {
